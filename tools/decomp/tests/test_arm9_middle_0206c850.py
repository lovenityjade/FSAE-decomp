...
CONFIG = PROJECT_ROOT / "config/decomp/arm9-middle-0206c850.json"
SOURCE = PROJECT_ROOT / "src/arm9/game/arm9_middle_0206c850_raw.c"
HEADER = PROJECT_ROOT / "include/game/arm9_middle_0206c850_raw.h"
...
class Arm9Middle0206c850RecoveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_unique_config_closes_at_32_functions(self) -> None:
        ...
        self.assertEqual(selection["entry_min"], "0x0206c850")
        self.assertEqual(selection["entry_max"], "0x0206d898")
        self.assertEqual(selection["last_body_end"], "0x0206d8f0")
        self.assertEqual(selection["next_entry"], "0x0206d8f0")
        self.assertEqual(selection["function_count"], 32)
        self.assertEqual(selection["body_bytes"], 4036)
        self.assertEqual(selection["pool_bytes"], 220)
        self.assertEqual(selection["span_bytes"], 4256)
        ...
        catalog_index = next(
            ...
            if int(item["entry"], 16) == 0x0206C850
        )
    ...
    def test_rom_body_pool_extent_hashes_and_boundaries(self) -> None:
        ...
        self.assertEqual(total_bodies, 4036)
        self.assertEqual(total_pools, 220)
        self.assertEqual(total_bodies + total_pools, 4256)
    ...
    def test_raw_bodies_match_export_with_only_documented_repairs(self) -> None:
        ...
        for address in selected:
            ...
            expected = expected.replace(
                "FUN_0206cea0();", "FUN_0206cea0(param_1);"
            )
            expected = expected.replace(
                "FUN_0206cef8();", "FUN_0206cef8(param_1);"
            )
            expected = expected.replace(
                "FUN_0206cb84();", "FUN_0206cb84(param_1);"
            )
            expected = expected.replace(
                "FUN_0204be88();", "FUN_0204be88(param_1);"
            )
            expected = expected.replace(
                "FUN_0206d350();", "FUN_0206d350(param_1);"
            )
            expected = expected.replace(
                "FUN_0206d5e0();", "FUN_0206d5e0(param_1);"
            )
            expected = expected.replace(
