...
CONFIG = PROJECT_ROOT / "config/decomp/arm9-middle-02065f5c.json"
SOURCE = PROJECT_ROOT / "src/arm9/game/arm9_middle_02065f5c_raw.c"
HEADER = PROJECT_ROOT / "include/game/arm9_middle_02065f5c_raw.h"
...
class Arm9Middle02065f5cRecoveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_unique_config_closes_at_32_functions(self) -> None:
        ...
        self.assertEqual(selection["entry_min"], "0x02065f5c")
        self.assertEqual(selection["entry_max"], "0x02067f78")
        self.assertEqual(selection["last_body_end"], "0x02067f90")
        self.assertEqual(selection["next_entry"], "0x02067f94")
        self.assertEqual(selection["function_count"], 32)
        self.assertEqual(selection["body_bytes"], 7520)
        self.assertEqual(selection["pool_bytes"], 728)
        self.assertEqual(selection["span_bytes"], 8248)
        ...
        catalog_index = next(
            ...
            if int(item["entry"], 16) == 0x02065F5C
        )
    ...
    def test_rom_body_pool_extent_hashes_and_boundaries(self) -> None:
        ...
        self.assertEqual(total_bodies, 7520)
        self.assertEqual(total_pools, 728)
        self.assertEqual(total_bodies + total_pools, 8248)

    def test_raw_source_and_header_own_exactly_32_entries(self) -> None:
        text = SOURCE.read_text(encoding="utf-8")
        definitions = re.findall(
            r"^(?:void|bool|ushort\s*\*)\s+FUN_(020[0-9a-f]{5})\([^;]*?\)\s*\{",
            text,
    ...
    def test_raw_bodies_match_export_with_only_documented_repairs(self) -> None:
        ...
        for address in selected:
            ...
            expected = expected.replace(
                "FUN_02067040();", "FUN_02067040(param_1);"
            )
            expected = expected.replace(
                "FUN_02067770();", "FUN_02067770(param_1);"
            )
            expected = expected.replace(
                "uVar1 = FUN_02066020(", "uVar1 = (undefined4)FUN_02066020("
            )
