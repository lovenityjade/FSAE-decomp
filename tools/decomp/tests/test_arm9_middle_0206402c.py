...
CONFIG = PROJECT_ROOT / "config/decomp/arm9-middle-0206402c.json"
SOURCE = PROJECT_ROOT / "src/arm9/game/arm9_middle_0206402c_raw.c"
HEADER = PROJECT_ROOT / "include/game/arm9_middle_0206402c_raw.h"
...
class Arm9Middle0206402cRecoveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_unique_config_closes_at_32_functions(self) -> None:
        ...
        self.assertEqual(selection["entry_min"], "0x0206402c")
        self.assertEqual(selection["entry_max"], "0x02065eb0")
        self.assertEqual(selection["last_body_end"], "0x02065f54")
        self.assertEqual(selection["next_entry"], "0x02065f5c")
        self.assertEqual(selection["function_count"], 32)
        self.assertEqual(selection["body_bytes"], 5080)
        self.assertEqual(selection["pool_bytes"], 2904)
        self.assertEqual(selection["span_bytes"], 7984)
        ...
        catalog_index = next(
            ...
            if int(item["entry"], 16) == 0x0206402C
        )
    ...
    def test_rom_body_pool_extent_hashes_and_boundaries(self) -> None:
        ...
        self.assertEqual(total_bodies, 5080)
        self.assertEqual(total_pools, 2904)
        self.assertEqual(total_bodies + total_pools, 7984)
    ...
    def test_raw_bodies_match_export_with_only_documented_repairs(self) -> None:
        ...
        for address in selected:
            ...
            expected = expected.replace(
                "FUN_020650f4();", "FUN_020650f4(param_1);"
            )
            expected = expected.replace(
                "FUN_02064fc0();", "FUN_02064fc0(param_1);"
            )
