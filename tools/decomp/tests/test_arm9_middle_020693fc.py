...
CONFIG = PROJECT_ROOT / "config/decomp/arm9-middle-020693fc.json"
SOURCE = PROJECT_ROOT / "src/arm9/game/arm9_middle_020693fc_raw.c"
HEADER = PROJECT_ROOT / "include/game/arm9_middle_020693fc_raw.h"
...
class Arm9Middle020693fcRecoveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_unique_config_closes_at_32_functions(self) -> None:
        ...
        self.assertEqual(selection["entry_min"], "0x020693fc")
        self.assertEqual(selection["entry_max"], "0x0206b460")
        self.assertEqual(selection["last_body_end"], "0x0206b4dc")
        self.assertEqual(selection["next_entry"], "0x0206b4dc")
        self.assertEqual(selection["function_count"], 32)
        self.assertEqual(selection["body_bytes"], 5732)
        self.assertEqual(selection["pool_bytes"], 2684)
        self.assertEqual(selection["span_bytes"], 8416)
        ...
        catalog_index = next(
            ...
            if int(item["entry"], 16) == 0x020693FC
        )
    ...
    def test_rom_body_pool_extent_hashes_and_boundaries(self) -> None:
        ...
        self.assertEqual(total_bodies, 5732)
        self.assertEqual(total_pools, 2684)
        self.assertEqual(total_bodies + total_pools, 8416)
    ...
    def test_raw_bodies_match_export_with_only_documented_repairs(self) -> None:
        ...
        for address in selected:
            ...
            expected = expected.replace(
                "FUN_0206b4dc();", "FUN_0206b4dc(param_1);"
            )
