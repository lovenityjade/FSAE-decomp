...
CONFIG = PROJECT_ROOT / "config/decomp/arm9-middle-0206e898.json"
SOURCE = PROJECT_ROOT / "src/arm9/game/arm9_middle_0206e898_raw.c"
HEADER = PROJECT_ROOT / "include/game/arm9_middle_0206e898_raw.h"
RAW_SHARDS = (
    PROJECT_ROOT / "build/decomp/arm9/modules/game_raw_0206.c",
    PROJECT_ROOT / "build/decomp/arm9/modules/game_raw_0207.c",
)
...
class Arm9Middle0206e898RecoveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_unique_config_closes_at_32_functions(self) -> None:
        ...
        self.assertEqual(selection["entry_min"], "0x0206e898")
        self.assertEqual(selection["entry_max"], "0x020709fc")
        self.assertEqual(selection["last_body_end"], "0x02070da0")
        self.assertEqual(selection["next_entry"], "0x02070dd0")
        self.assertEqual(selection["function_count"], 32)
        self.assertEqual(selection["body_bytes"], 5152)
        self.assertEqual(selection["pool_bytes"], 4376)
        self.assertEqual(selection["span_bytes"], 9528)
        ...
        catalog_index = next(
            ...
            if int(item["entry"], 16) == 0x0206E898
        )
    ...
    def test_rom_body_pool_extent_hashes_and_boundaries(self) -> None:
        ...
        self.assertEqual(total_bodies, 5152)
        self.assertEqual(total_pools, 4376)
        self.assertEqual(total_bodies + total_pools, 9528)

    def test_raw_source_and_header_own_exactly_32_entries(self) -> None:
        text = SOURCE.read_text(encoding="utf-8")
        definitions = re.findall(
            r"^(?:void|bool|ushort\s*\*|undefined4|uint)\s+FUN_(020[0-9a-f]{5})\([^;]*?\)\s*\{",
            text,
        ...
        definition = re.compile(
            r"^(?:void|bool|ushort\s*\*|undefined4|uint)\s+"
    ...
    def test_raw_bodies_match_export_with_only_documented_repairs(self) -> None:
        ...
        for path in RAW_SHARDS:
            exported.update(blocks(path.read_text(encoding="utf-8")))
        ...
        for address in selected:
            ...
            expected = expected.replace(
                "FUN_0204be88();", "FUN_0204be88(param_1);"
            )
            expected = expected.replace(
                "FUN_02085980();", "FUN_02085980(param_1);"
            )
            expected = expected.replace(
                "FUN_020859e0();", "FUN_020859e0(param_1);"
            )
