...
CONFIG = PROJECT_ROOT / "config/decomp/arm9-middle-0206b4dc.json"
SOURCE = PROJECT_ROOT / "src/arm9/game/arm9_middle_0206b4dc_raw.c"
HEADER = PROJECT_ROOT / "include/game/arm9_middle_0206b4dc_raw.h"
...
class Arm9Middle0206b4dcRecoveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_unique_config_closes_at_32_functions(self) -> None:
        ...
        self.assertEqual(selection["entry_min"], "0x0206b4dc")
        self.assertEqual(selection["entry_max"], "0x0206c7cc")
        self.assertEqual(selection["last_body_end"], "0x0206c84c")
        self.assertEqual(selection["next_entry"], "0x0206c850")
        self.assertEqual(selection["function_count"], 32)
        self.assertEqual(selection["body_bytes"], 4532)
        self.assertEqual(selection["pool_bytes"], 448)
        self.assertEqual(selection["span_bytes"], 4980)
        ...
        catalog_index = next(
            ...
            if int(item["entry"], 16) == 0x0206B4DC
        )
    ...
    def test_rom_body_pool_extent_hashes_and_boundaries(self) -> None:
        ...
        self.assertEqual(total_bodies, 4532)
        self.assertEqual(total_pools, 448)
        self.assertEqual(total_bodies + total_pools, 4980)
    ...
    def test_raw_bodies_match_export_with_only_documented_repairs(self) -> None:
        ...
        for address in selected:
            ...
            expected = expected.replace(
                "FUN_0206b768();", "FUN_0206b768(param_1);"
            )
            expected = expected.replace(
                "FUN_0206bae4();", "FUN_0206bae4(param_1);"
            )
            expected = expected.replace(
                "FUN_0206c18c();", "FUN_0206c18c(param_1);"
            )
            expected = expected.replace(
                "FUN_0206c4c0();", "FUN_0206c4c0(param_1);"
            )
            expected = expected.replace(
