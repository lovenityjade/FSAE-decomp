...
CONFIG = PROJECT_ROOT / "config/decomp/arm9-middle-0207303c.json"
SOURCE = PROJECT_ROOT / "src/arm9/game/arm9_middle_0207303c_raw.c"
HEADER = PROJECT_ROOT / "include/game/arm9_middle_0207303c_raw.h"
...
class Arm9Middle0207303cRecoveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_unique_config_closes_at_32_functions(self) -> None:
        ...
        self.assertEqual(selection["entry_min"], "0x0207303c")
        self.assertEqual(selection["entry_max"], "0x02073e60")
        self.assertEqual(selection["last_body_end"], "0x02073ef4")
        self.assertEqual(selection["next_entry"], "0x02073efc")
        self.assertEqual(selection["function_count"], 32)
        self.assertEqual(selection["body_bytes"], 3252)
        self.assertEqual(selection["pool_bytes"], 524)
        self.assertEqual(selection["span_bytes"], 3776)
        ...
        catalog_index = next(
            ...
            if int(item["entry"], 16) == 0x0207303C
        )
    ...
    def test_rom_body_pool_extent_hashes_and_boundaries(self) -> None:
        ...
        self.assertEqual(total_bodies, 3252)
        self.assertEqual(total_pools, 524)
        self.assertEqual(total_bodies + total_pools, 3776)
    ...
    def test_raw_bodies_match_export_with_only_documented_repairs(self) -> None:
        ...
        for address in selected:
            ...
            expected = expected.replace(
                "FUN_020742cc();", "FUN_020742cc(param_1);"
            )
            expected = expected.replace(
                "FUN_02085d34();", "FUN_02085d34(param_1);"
            )
            expected = expected.replace(
                "FUN_0205063c();", "FUN_0205063c(puVar1,param_1);"
            )
            if address != 0x02073C88:
                expected = expected.replace(
