...
PUBLIC_MANIFEST = PROJECT_ROOT / "config/decomp/arm9-sdk-0200f9fc.json"
MANIFEST = (
    PUBLIC_MANIFEST
    if PUBLIC_MANIFEST.exists()
    else DRAFT_ROOT / "arm9-sdk-0200f9fc.json"
)
ARM9_BASE = 0x02004000


class SdkBlock0200f9fcRecoveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_catalog_hashes_extents_and_checkpoint_metrics(self) -> None:
        ...
        self.assertEqual(selection["entry_min"], "0x0200f9fc")
        self.assertEqual(selection["entry_max"], "0x02010ba0")
        self.assertEqual(selection["next_entry"], "0x02010bb4")
        ...
        start = next(
            ...
            if item["entry"] == "0x0200f9fc"
        )
        catalog_selected = catalog_functions[start : start + 32]
        functions = self.manifest["functions"]
        self.assertEqual(catalog_functions[start + 32]["entry"], "0x02010bb4")
        self.assertEqual(
        ...
        self.assertEqual(body_total, 4484)
        ...
        self.assertEqual(selection["span_bytes"], 4536)

    def test_existing_sdk_signatures_own_every_byte_without_semantic_duplicates(self) -> None:
        signatures: dict[int, list[dict[str, object]]] = {}
        for item in self.signatures["matches"]:
            signatures.setdefault(item["address"], []).append(item)
        semantic = json.loads(
        ...
        for function in self.manifest["functions"]:
            ...
            candidates = signatures[address]
            matching = [
                item for item in candidates if item["name"] == function["name"]
            ]
            with self.subTest(function=function["name"]):
                self.assertNotIn(address, semantic_addresses)
                self.assertEqual(len(matching), 1)
                signature = matching[0]
                if "boundary_candidate_names" in provenance:
                    self.assertEqual(
                        sorted(item["name"] for item in candidates),
                        sorted(provenance["boundary_candidate_names"]),
                    )
                    self.assertEqual(
                        len(candidates), provenance["boundary_candidate_count"]
                    )
                    self.assertEqual(
                        signature["name"], provenance["selected_signature_name"]
                    )
            ...
            matched_total += matching[0]["matched_bytes"]
            masked_total += matching[0]["relocation_masked_bytes"]

        self.assertEqual(matched_total, 4044)
        self.assertEqual(masked_total, 492)
        self.assertEqual(matched_total + masked_total, 4536)
