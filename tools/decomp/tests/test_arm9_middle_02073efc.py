...
CONFIG = PROJECT_ROOT / "config/decomp/arm9-middle-02073efc.json"
SOURCE = PROJECT_ROOT / "src/arm9/game/arm9_middle_02073efc_raw.c"
HEADER = PROJECT_ROOT / "include/game/arm9_middle_02073efc_raw.h"
...
class Arm9Middle02073efcRecoveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_unique_config_closes_at_32_functions(self) -> None:
        ...
        self.assertEqual(selection["entry_min"], "0x02073efc")
        self.assertEqual(selection["entry_max"], "0x0207543c")
        self.assertEqual(selection["last_body_end"], "0x020754a8")
        self.assertEqual(selection["next_entry"], "0x020754ac")
        self.assertEqual(selection["function_count"], 32)
        self.assertEqual(selection["body_bytes"], 4376)
        self.assertEqual(selection["pool_bytes"], 1176)
        self.assertEqual(selection["span_bytes"], 5552)
        ...
        catalog_index = next(
            ...
            if int(item["entry"], 16) == 0x02073EFC
        )
    ...
    def test_rom_body_pool_extent_hashes_and_boundaries(self) -> None:
        ...
        self.assertEqual(total_bodies, 4376)
        self.assertEqual(total_pools, 1176)
        self.assertEqual(total_bodies + total_pools, 5552)
    ...
    def test_raw_bodies_match_export_with_only_documented_repairs(self) -> None:
        ...
        for address in selected:
            ...
            expected = expected.replace(
                "FUN_020768d8();", "FUN_020768d8(iVar1);"
            )
