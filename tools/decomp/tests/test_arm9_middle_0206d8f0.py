...
CONFIG = PROJECT_ROOT / "config/decomp/arm9-middle-0206d8f0.json"
SOURCE = PROJECT_ROOT / "src/arm9/game/arm9_middle_0206d8f0_raw.c"
HEADER = PROJECT_ROOT / "include/game/arm9_middle_0206d8f0_raw.h"
...
class Arm9Middle0206d8f0RecoveryTests(unittest.TestCase):
    @classmethod
    ...
    def test_unique_config_closes_at_32_functions(self) -> None:
        ...
        self.assertEqual(selection["entry_min"], "0x0206d8f0")
        self.assertEqual(selection["entry_max"], "0x0206e688")
        self.assertEqual(selection["last_body_end"], "0x0206e86c")
        self.assertEqual(selection["next_entry"], "0x0206e898")
        self.assertEqual(selection["function_count"], 32)
        self.assertEqual(selection["body_bytes"], 3608)
        self.assertEqual(selection["pool_bytes"], 400)
        self.assertEqual(selection["span_bytes"], 4008)
        ...
        catalog_index = next(
            ...
            if int(item["entry"], 16) == 0x0206D8F0
        )
    ...
    def test_rom_body_pool_extent_hashes_and_boundaries(self) -> None:
        ...
        self.assertEqual(total_bodies, 3608)
        self.assertEqual(total_pools, 400)
        self.assertEqual(total_bodies + total_pools, 4008)
    ...
    def test_raw_bodies_match_export_with_only_documented_repairs(self) -> None:
        ...
        for address in selected:
            ...
            expected = expected.replace(
                "FUN_0206dcc8();", "FUN_0206dcc8(param_1);"
            )
            expected = expected.replace(
                "FUN_0206e1b8();", "FUN_0206e1b8(param_1);"
            )
            expected = expected.replace(
                "FUN_0206e210();", "FUN_0206e210(param_1);"
            )
            expected = expected.replace(
